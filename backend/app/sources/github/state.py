...
def sync_github_baseline_for_profile(profile: SubscriptionQueryProfile) -> None:
...
def load_watched_github_repositories(subscription_id: str) -> tuple[Repository, ...]:
    """Load watched GitHub repositories for one subscription."""

    matches = list_subscription_repository_matches(subscription_id)
    repository_ids = [
        match.repository_id for match in matches if match.source == "github"
    ]
...
def describe_watched_github_repositories(subscription_id: str) -> list[dict[str, object]]:
    ...
    repositories = load_watched_github_repositories(subscription_id)
    return [
        {
