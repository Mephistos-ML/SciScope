...
def sync_gitlab_baseline_for_profile(profile: ResearchProfile) -> None:
    """Initialize checkpoints for watched GitLab projects that have none yet."""

    watched_projects = load_watched_gitlab_repository_entities(profile.topic_slug)
    checkpoints_to_upsert: list[EntityCheckpoint] = []

    for entity in watched_projects:
...
def load_watched_gitlab_repository_entities(topic_slug: str) -> tuple[Entity, ...]:
    """Load watched GitLab repository entities for one topic."""

    matches = list_topic_entity_matches(topic_slug)
    entity_ids = [match.entity_id for match in matches if match.source == "gitlab"]
    entities = list_entities_by_ids(entity_ids)

    repositories: list[Entity] = []
    for entity in entities:
        if entity.source != "gitlab" or entity.entity_type != "repository":
            continue
...
def describe_watched_gitlab_repositories(topic_slug: str) -> list[dict[str, object]]:
    """Return watched GitLab repository metadata for debug visibility."""

    repositories = load_watched_gitlab_repository_entities(topic_slug)
    return [
        {
...
def resolve_release_checkpoint(
    ...
    *,
    ...
    """Resolve the monitoring cursor for one watched GitLab entity."""
